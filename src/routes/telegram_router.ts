...
router.post("/message-to-telegram", apiKeyAuth, async (req: Request, res: Response) => {
  try {
    const { chat, message } = req.body;
    if (!chat || !message) {
      return res.status(400).send("Missing 'chat' or 'message' in request body");
    }
    
    const telegramMessageId = await telegramService.sendMessageToTelegram(chat, message);
